...
namespace ASCOM.autoFilterWheel.FilterWheel
{
    ...
    public class FilterWheel : ReferenceCountedObjectBase, IFilterWheelV3, IDisposable
    {
        ...
        public void Disconnect()
        {
            try
            {
                LogMessage("Disconnect", "Disconnecting from device");
                Connected = false;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Returns true while the device is connecting (always false for this synchronous driver)
        /// </summary>
        public bool Connecting
        {
            get
            {
                try
                {
                    LogMessage("Connecting Get", "false");
                    return false; // This driver connects synchronously
                }
                catch (Exception ex)
                {
                    LogMessage("Connecting Get", $"Threw an exception: \r\n{ex}");
                    throw;
                }
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                try
                {
                    LogMessage("DeviceState Get", "Returning device state collection");
                    return FilterWheelHardware.DeviceState;
                }
                catch (Exception ex)
                {
                    LogMessage("DeviceState Get", $"Threw an exception: \r\n{ex}");
                    throw;
                }
            }
        }
